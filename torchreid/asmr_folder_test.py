...
from .reid_dataset import import_pa100k
#from .reid_dataset import import_DukeMTMCAttribute_binary
#from .reid_dataset import import_DukeMTMC
import os.path as osp
from collections import Counter
import pdb
import pickle

from .transforms import RandomEraising 
...
class Base_Dataset(data.Dataset):
    def __init__(self, data_dir, dataset_name, num_train, seed):
        ...
        if dataset_name == 'Market-1501':
            self.sem_attr=[[] for i in range(508)]
            ...
            #pdb.set_trace()
            self.word2vec = np.load('/media/data2/pengqy/SAL/utils/market_glove.npy', allow_pickle=True)
            self.glove = np.load('/media/data2/pengqy/SAL/dataset/glove-market.npy', encoding="latin1",allow_pickle=True).item()
            for k in self.glove.keys():
                
                for i in range(len(self.glove[k])):
                    if isinstance(self.glove[k][i], list):
                        self.glove[k][i] = np.sum(self.glove[k][i][:], axis=0)
            ...
            cc = np.array(cc)
            pdb.set_trace()
            sum=np.sum(cc,axis=0)
            #print(sum)
            concur=np.zeros((30, 30))
            #print(concur)
            for k in range(508):
                for i in range(30):
                    if cc[k][i]==1:
            ...
            with open('dataset/adj_market.pkl','wb+') as f:
                pickle.dump(data,f)
            pdb.set_trace()
            '''
        elif dataset_name == 'PETA':
            self.sem_attr=[[] for i in range(1890)]
            self.train, self.query, self.gallery, self.train_attr, self.test_attr, self.label = import_peta(data_dir)
            #pdb.set_trace()
            self.word2vec = np.load('/media/data2/pengqy/TIP/dataset/PETA/peta_glove.npy', allow_pickle=True)
            self.glove = np.load('/media/data2/pengqy/SAL/dataset/glove-market.npy', encoding="latin1",allow_pickle=True).item()
            for k in self.glove.keys():   
                for i in range(len(self.glove[k])):
                    if isinstance(self.glove[k][i], list):
            ...
            sum=np.sum(cc,axis=0)
            #print(sum)
            concur=np.zeros((105, 105))
            #print(concur)
            for k in range(12140):
                for i in range(105):
                    if cc[k][i]==1:
                        for j in range(105):
                            if cc[k][j]==1 and j!=i:
            ...
            self.train, self.query, self.gallery, self.train_attr, self.test_attr, self.label = import_pa100k(data_dir)
            self.sem_attr=[[] for i in range(2020)]
            #pdb.set_trace()
            self.word2vec = np.load('/media/data2/pengqy/TIP/dataset/PA100K/pa100k_glove.npy', allow_pickle=True)
            self.glove = np.load('/media/data2/pengqy/SAL/dataset/glove-market.npy', encoding="latin1",allow_pickle=True).item()
            for k in self.glove.keys():   
                for i in range(len(self.glove[k])):
                    if isinstance(self.glove[k][i], list):
            ...
            self.sem_attr=[[] for i in range(299)]
            self.train, self.query, self.gallery = import_MarketDuke_nodistractors(data_dir, dataset_name)
            self.train_attr, self.test_attr, self.label = import_DukeMTMCAttribute_binary(data_dir)
            #pdb.set_trace()
            self.word2vec = np.load('/media/data1/pengqy/attributed_based_person_seach/SAL/utils/pa100k_glove.npy', allow_pickle=True)
            self.glove = np.load('/media/data1/pengqy/attributed_based_person_seach/SAL/dataset/glove-market.npy', encoding="latin1",allow_pickle=True).item()
            for k in self.glove.keys():   
                for i in range(len(self.glove[k])):
                    if isinstance(self.glove[k][i], list):
                        self.glove[k][i] = np.sum(self.glove[k][i][:], axis=0)    
        ...
        for k, v in self.train_attr.items():
            ...
            if semantic_id in semantic_dict:
                ...
                self.sem_attr[num_count] = np.array(v)
...
class Train_Dataset():
    def __init__(self, base, transforms=None, train_val='train', glove_start=None ):
        ...
        self.glove_start = glove_start
        self.word2vec = base.word2vec
        if self.glove_start != None:
            self.glove_embedding  = base.glove
    ...
    def __getitem__(self, index):
        ...
        #pdb.set_trace()
        if self.glove_start!=None:
            return data, i, label, id, cam, name, sem, label[:, np.newaxis]*self.word2vec[1] + (1-label[:, np.newaxis])*self.word2vec[0]
            #return data, i, label, id, cam, name, sem, self.glove_embedding[str(int(id))]
...
class Test_Dataset():
    def __init__(self, base, transforms=None, query_gallery='gallery' ,glove_start=None):
        #pdb.set_trace()
        ...
        if query_gallery == 'query':
            ...
            if base.dataset_name == 'PETA' or  base.dataset_name == 'PA100K':
                self.test_data = gallery['data']
            else:
    ...
    def __getitem__(self, index):
        ...
        #pdb.set_trace()
        if self.glove_start!=None:
            return data, label, id, camid, img_path, sem, label[:, np.newaxis]*self.word2vec[1] + (1-label[:, np.newaxis])*self.word2vec[0]
            #return data, label, id, camid, img_path, sem, self.glove_embedding[str(int(id))]
