...
from models import model_glove_extra, model_d, model_x, model_a, model_i2, model_da, model_ga, model_dx, model_gx
...
def main():
    ...
    model_G = model_glove_extra(args.extra_num).to(device)
    ...
    if args.evaluate:
        ...
        for name in args.dataset:
            ...
            checkpoint = torch.load('/public/users/pengqy/log/market_glove_extra_1/checkpoint_ep_al_I1best.pth.tar')
            model_I1.module.load_state_dict(checkpoint['state_dict'])
           
            checkpoint = torch.load('/public/users/pengqy/log/market_glove_extra_1/checkpoint_ep_al_xbest.pth.tar')
            model_xencoder.module.load_state_dict(checkpoint['state_dict']) 

            checkpoint = torch.load('/public/users/pengqy/log/market_glove_extra_1/checkpoint_ep_al_abest.pth.tar')
            model_aencoder.module.load_state_dict(checkpoint['state_dict'])   
                       
            checkpoint = torch.load('/public/users/pengqy/log/market_glove_extra_1/checkpoint_ep_al_Gbest.pth.tar')
            model_G.module.load_state_dict(checkpoint['state_dict']) 
...
def train_I(epoch, model_I1, model_I2, model_I2_sem, model_xencoder, criterion, criterion_sem, optimizer, trainloader,
    ...
    for batch_idx, (imgs, _, attrs, _, _, _, sems,_) in enumerate(trainloader):
...
def train_WOadv(epoch, model_I1, model_I2, model_I2_sem, model_G, model_xencoder, model_aencoder, criterion,
    ...
    for batch_idx, (imgs, _, attrs, _, _, _, sems, glove) in enumerate(trainloader):
        ...
        if use_gpu:
            imgs, attrs, sems,glove = imgs.cuda(), attrs.cuda(), sems.cuda(), glove.cuda().float()
...
def test_WOadv(model_I1, model_G, model_xencoder, model_aencoder, queryloader, galleryloader, use_gpu,
               ranks=[1, 5, 10, 20], return_distmat=False,  test_origin=None):
    batch_time = AverageMeter()
