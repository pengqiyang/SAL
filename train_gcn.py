...
from torchreid.asmr_data_manager import ImageDataManager
...
from models import model_g, model_d, model_x, model_a, model_i2, model_da, model_ga, model_dx, model_gx, GCNResnet_extra
...
def main():
    ...
    if args.evaluate:
        ...
        for name in args.dataset:
            ...
            checkpoint = torch.load('/public/users/pengqy/log/peta_45/checkpoint_ep_al_I1best.pth.tar')
            model_I1.module.load_state_dict(checkpoint['state_dict'])
           
            checkpoint = torch.load('/public/users/pengqy/log/peta_45/checkpoint_ep_al_xbest.pth.tar')
            model_xencoder.module.load_state_dict(checkpoint['state_dict']) 

            checkpoint = torch.load('/public/users/pengqy/log/peta_45/checkpoint_ep_al_abest.pth.tar')
            model_aencoder.module.load_state_dict(checkpoint['state_dict'])   
                       
            checkpoint = torch.load('/public/users/pengqy/log/peta_45/checkpoint_ep_al_Gbest.pth.tar')
            model_G.module.load_state_dict(checkpoint['state_dict']) 
            distmat = test_WOadv(model_I1, model_G, model_xencoder, model_aencoder, queryloader, galleryloader, use_gpu,
                                 return_distmat=True, test_origin=True)
...
def test_WOadv(model_I1, model_G, model_xencoder, model_aencoder, queryloader, galleryloader, use_gpu,
    ...
    cmc, mAP = evaluate(distmat, q_pids, g_pids, q_camids, g_camids, use_metric_cuhk03=False, test_origin=test_origin)
