...
from args import argument_parser, image_dataset_kwargs, optimizer_kwargs, optimizer_kwargs_sc, optimizer_kwargs_pt, \
    optimizer_kwargs_al, optimizer_kwargs_gf, optimizer_kwargs_df,  optimizer_kwargs_al_alone, optimizer_kwargs_sc_alone
from torchreid.asmr_data_manager_test import ImageDataManager
...
from torchreid.eval_metrics_test import evaluate
from torchreid.optimizers import init_optimizer
from models_test import model_g, model_d, model_x, model_a, model_i2, model_da, model_ga, model_dx, model_gx, GCNResnet_extra
...
def main():
    ...
    model_G = GCNResnet_extra(extra_num=args.extra_num, t=args.t, num_class=dm.num_train_attrs, adj_file=args.adj_file, adj_relation=1, adj_norelation=0, adj_init=args.adj_init, extra_adj = args.extra_adj).to(device)
    ...
    optimizer_G = init_optimizer(
        list(model_aencoder.parameters()) + list(model_I2.parameters()) + list(
            model_I2_sem.parameters()), **optimizer_kwargs_sc(args))
    optimizer_G_alone = init_optimizer(list(model_G.parameters()) , **optimizer_kwargs_sc_alone(args))
    ...
    optimizer_A_alone = init_optimizer(list(model_G.parameters()) , **optimizer_kwargs_al_alone(args))
    ...
    if args.evaluate:
        ...
        for name in args.dataset:
            ...
            checkpoint = torch.load('log/test/market/checkpoint_ep_jt_I1best.pth.tar')
            model_I1.module.load_state_dict(checkpoint['state_dict'])
           
            checkpoint = torch.load('log/test/market/checkpoint_ep_jt_xbest.pth.tar')
            model_xencoder.module.load_state_dict(checkpoint['state_dict']) 

            checkpoint = torch.load('log/test/market/checkpoint_ep_jt_abest.pth.tar')
            model_aencoder.module.load_state_dict(checkpoint['state_dict'])   
                       
            checkpoint = torch.load('log/test/market/checkpoint_ep_jt_Gbest.pth.tar')
            model_G.module.load_state_dict(checkpoint['state_dict']) 
            #distmat = test_WOadv(model_I1, model_G, model_xencoder, model_aencoder, queryloader, galleryloader, use_gpu,
            #                    return_distmat=True, test_origin=False)
            distmat = grad(model_I1, model_G, model_xencoder, model_aencoder, queryloader, galleryloader, use_gpu,
                                 return_distmat=True, test_origin=False)
    ...
    scheduler_G_alone = lr_scheduler.MultiStepLR(optimizer_G_alone, milestones=args.stepsize, gamma=args.sc_alone_gamma)
    ...
    scheduler_A_alone = lr_scheduler.MultiStepLR(optimizer_A_alone, milestones=args.stepsize_sal, gamma=args.al_alone_gamma)


    word2vec = torch.from_numpy(np.array(dm.train_dataset.word2vec)).cuda()
    ...
    for epoch in range(args.start_epoch_jt, args.max_epoch_jt):
        ...
        train_WOadv(epoch, model_I1, model_I2, model_I2_sem, model_G, model_xencoder, model_aencoder, criterion,
                    ...
                    optimizer_G, optimizer_G_alone, trainloader, use_gpu, fixbase=False)
        train_time += round(time.time() - start_train_time)

        scheduler_G.step()
        scheduler_G_alone.step()
        scheduler_I.step()
        ...
        if (epoch + 1) % args.save_pt == 0 or  (rank1 > best_rank1):
            is_best = False
            if rank1 > best_rank1:
                best_rank1 = rank1
                is_best = True
                ...
                save_checkpoint({
                    ...
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_jt_I1' + str('best') + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_I2,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_jt_I2' + str('best') + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_I2_sem,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_jt_I2sem' + str('best') + '.pth.tar'))
                ...
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_jt_G' + str('best') + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_xencoder,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_jt_x' + str('best') + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_aencoder,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_jt_a' + str('best') + '.pth.tar'))
        print(best_rank1)
    elapsed = round(time.time() - start_time)
    elapsed = str(datetime.timedelta(seconds=elapsed))
    train_time = str(datetime.timedelta(seconds=train_time))
    ...
    for epoch in range(args.start_epoch_al, args.max_epoch_al):
        ...
        train_gan(epoch, w, average, model_I1, model_I2, model_I2_sem, model_G, model_xencoder, model_aencoder, model_GA,
                  ...
                  optimizer_DS,
                  optimizer_ES, optimizer_X, optimizer_A, optimizer_A_alone, optimizer_DC, trainloader, use_gpu,
                  fixbase=False, word2vec=word2vec)
        train_time += round(time.time() - start_train_time)

        scheduler_X.step()
        scheduler_A.step()
        scheduler_A_alone.step()
        scheduler_DS.step()
        scheduler_ES.step()
        scheduler_DC.step()
        scheduler_GC.step()
        ...
        if (epoch + 1) % args.save_pt == 0 or (rank1 > best_rank1):
            ...
            if rank1 > best_rank1:
                ...
                save_checkpoint({
                    ...
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_I1' + str('best') + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_G,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_G' + str('best') + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_I2,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_I2' + str('best') + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_I2_sem,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_I2sem' + str('best') + '.pth.tar'))
                ...
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_x' + str('best') + '.pth.tar'))
                ...
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_a' + str('best') + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_GX,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_GX' + str('best') + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_GA,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_GA' + str('best') + '.pth.tar'))
                ...
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_DS' + str('best') + '.pth.tar'))
                ...
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_DC' + str('best') + '.pth.tar'))
...
def train_I(epoch, model_I1, model_I2, model_I2_sem, model_xencoder, criterion, criterion_sem, optimizer, trainloader,
    ...
    for batch_idx, (imgs, _, attrs, _, _, _, sems, glove) in enumerate(trainloader):
        data_time.update(time.time() - end)

        if use_gpu:
            imgs, attrs, sems, glove = imgs.cuda(), attrs.cuda(), sems.cuda(),glove.reshape(sems.size(0),-1,50).cuda().float()
...
def train_gan(epoch, w, average, model_I1, model_I2, model_I2_sem, model_G, model_xencoder, model_aencoder, model_GA, model_GX,
    ...
    if epoch >= args.epoch_add_fake:
        for batch_idx, (imgs, _, attrs, _, _, _, sems, glove) in enumerate(trainloader):
            ...
            sems = sems.long()
            attrs = attrs.float()
            glove = glove.reshape(sems.size(0),-1,50).cuda().float()
            fake_attrs = fake_attrs.float()
            #pdb.set_trace()
            fake_glove = fake_attrs.unsqueeze(2) * word2vec[1].unsqueeze(0) + (1-fake_attrs.unsqueeze(2))*word2vec[0].unsqueeze(0)

            #############################
            #  train the image network  #
            #############################
            x_open = model_I1(imgs)
            a_open = model_G(glove)
            faa_open = model_G(fake_glove)
            x = x_open.detach()
            a = a_open.detach()
            faa = faa_open.detach()
...
def fake_attributes(w, average, batch_size):
    ...
    if dataset == 'Market-1501':
        ...
        if args.prob=='prior':
            #pdb.set_trace()
            w_bin = np.hstack((w[0], w[7:13],w[21], w[25]))
            #w_bin = np.concatenate((w[0], w[7:13],w[21], w[25])) # 9
            bin_prob = torch.tensor(w_bin)
            age_prob = torch.tensor(w[1:4]) # 3
            sleeve_prob = torch.tensor(w[13:15]) # 2
            up_prob = torch.tensor(w[15:19])     # 4      
            down_prob = torch.tensor(w[19:21])  # 2
            lower_prob = torch.tensor(w[22:25]) # 3
            dir_prob = torch.tensor(w[4:7])# 3
        ...
        sleeve_dist = torch.distributions.categorical.Categorical(sleeve_prob / sleeve_prob.sum())
        sleeve = sleeve_dist.sample(sample_shape=(batch_size,)).long()
        ...
        lower_dist = torch.distributions.categorical.Categorical(lower_prob / lower_prob.sum())
        lower = lower_dist.sample(sample_shape=(batch_size,)).long()


        # actually no need to divide by the sum of probabilities.
        dir_dist = torch.distributions.categorical.Categorical(dir_prob / dir_prob.sum())
        dire = dir_dist.sample(sample_shape=(batch_size,)).long()


  
        bin_vector = torch.zeros(len(bin), len(w_bin)).scatter_(1, bin, 1.)
        age_one_hot = torch.zeros(len(age), 3).scatter_(1, age.unsqueeze(1), 1.)
        down_one_hot = torch.zeros(len(down), 2).scatter_(1, down.unsqueeze(1), 1.)
        up_one_hot = torch.zeros(len(up), 4).scatter_(1, up.unsqueeze(1), 1.)
        sleeve_one_hot = torch.zeros(len(sleeve), 2).scatter_(1, sleeve.unsqueeze(1), 1.)
        dire_one_hot = torch.zeros(len(dire), 3).scatter_(1, dire.unsqueeze(1), 1.)
        lower_one_hot = torch.zeros(len(lower), 3).scatter_(1, lower.unsqueeze(1), 1.)
        #pdb.set_trace()
        fake_attrs = torch.cat((bin_vector[:,0].reshape(-1,1), age_one_hot, dire_one_hot, bin_vector[:,1:7].reshape(-1,6), sleeve_one_hot, up_one_hot, down_one_hot, bin_vector[:,7].reshape(-1,1), lower_one_hot, bin_vector[:,8].reshape(-1,1)), 1)
...
def test_WOadv(model_I1, model_G, model_xencoder, model_aencoder, queryloader, galleryloader, use_gpu,
               ranks=[1, 5, 10, 20], return_distmat=False, test_origin=False):
    batch_time = AverageMeter()
    ...
    model_xencoder.eval()
    model_aencoder.eval()
    sems_id = defaultdict(list)
    with torch.no_grad():
        qf, q_pids, q_camids, q_path, q_attrs = [], [], [], [], []
        for batch_idx, (_, attrs, ids, camids, names, sems, glove) in enumerate(queryloader):
            if use_gpu: attrs = attrs.cuda()
            glove = glove.reshape(sems.size(0), -1, 50).cuda().float().cuda()
            attrs = attrs.float()
            sems_id[int(sems[0])].append(attrs[0].data)
            end = time.time()
            
            features = model_aencoder(model_G(glove))
            ...
            q_path.extend(names)
            q_attrs.extend(attrs.cpu().numpy())
        ...
        q_attrs = np.asarray(q_attrs)
        
        np.save('src/pa100k_extra_text_feature_late.npy', qf)
        #np.save('src/pa100k_pids.npy', q_pids)
        #np.save('src/pa100k_sems_id.npy', sems_id)
        #np.save('src/pa100k_query_path.npy', q_path)
        return 
        print("Extracted features for query set, obtained {}-by-{} matrix".format(qf.size(0), qf.size(1)))

        gf, g_pids, g_camids, g_path, g_attrs = [], [], [], [], []
        end = time.time()
        # data, i, label, id, cam, name, sem
        for batch_idx, (imgs, attrs, id, camids, name, sems,_) in enumerate(galleryloader):
            if use_gpu:
                sems = sems.cuda()
                imgs = imgs.cuda()
            ...
            g_path.extend(name)
            g_attrs.extend(attrs.cpu().numpy())
        gf = torch.cat(gf, 0)
        g_pids = np.asarray(g_pids)
        g_camids = np.asarray(g_camids)
        g_path = np.asarray(g_path)
        g_attrs = np.asarray(g_attrs)
    ...
    cmc, mAP = evaluate(distmat, q_pids, g_pids, q_camids, g_camids, q_path, g_path, q_attrs, g_attrs, use_metric_cuhk03=False, test_origin=test_origin)
...
class Grad_Cam:
    def __init__(self, model_I1,model_xencoder, target_layer_names, use_cuda, path):
        self.model_I1 = model_I1
        self.model_xencoder = model_xencoder
        #self.fc = torch.nn.Parameter(torch.FloatTensor(512, 200), requires_grad=True)
        self.fc = np.load(path)#
        self.fc = torch.nn.Parameter(torch.FloatTensor(self.fc).cuda(), requires_grad=True).permute(1,0)
    ...
    def _normalize(self,cam):
        ...
        #pdb.set_trace()
        cam = heatmap + np.float32(cv2.resize(self.o_i, (w,h)))/255
        cam = cam / np.max(cam)
    ...
    def __call__(self, img, name,  idx=None):
        """
        :param inputs: [w,h,c]
        :param idx: class id
        ...
        """
        #pdb.set_trace()
        self.o_i = cv2.imread(name)
        #pdb.set_trace()
        self.model_I1.zero_grad()
        self.model_xencoder.zero_grad()
        
        #self.fc.zero_grad()
        #pdb.set_trace()
        if self.use_cuda:
            self.inputs = img
            self.model_xencoder = self.model_xencoder.cuda()
            self.model_I1 = self.model_I1.cuda()
            self.fc = self.fc.cuda()
        #pdb.set_trace()
        output = torch.matmul(F.normalize(self.model_xencoder(self.model_I1(self.inputs))), F.normalize(self.fc,dim=0))
        #pdb.set_trace()
        #if idx is None:
        predict = np.argmax(output.detach().cpu().numpy()) #predict id
        global result
        result[int(idx)].add(predict)
        #pdb.set_trace()
        if idx == predict:
            global total
            total = total+1
        #print(total)
        #print(str(idx) + " " + str(predict))
        target = output[0][idx]
        target.backward()
        #computer 
        weights = []
...
def grad(model_I1, model_G, model_xencoder, model_aencoder, queryloader, galleryloader,
                                   use_gpu, ranks=[1, 5, 10, 20], return_distmat=False, test_origin=None):
    batch_time = AverageMeter()
    ...
    model_xencoder.eval()
    model_aencoder.eval()
    true_path = np.load('src/market_extra_predict_true_path.npy')
    false_path = np.load('src/market_predict_true_path.npy')
    if True:
        aa = 1
    #with torch.no_grad():
        gf, g_pids, g_camids, g_path, g_attrs = [], [], [], [], []
        ...
        #pdb.set_trace()
        # data, i, label, id, cam, name, sem
        for batch_idx, (imgs, attrs, id, camids, name, sems, glove) in enumerate(galleryloader):
            if use_gpu:
                #glove = torch.stack(glove, 0).permute(1,0,2).reshape(sems.size(0),250).cuda()
                glove = glove.reshape(sems.size(0), -1, 50).cuda().float()
                ...
                #print(imgs[0][0])
                #print("origin/"+name[0].split('/')[3])
                #pdb.set_trace()
                #print(name)
                if (name[0] in true_path):
                    shutil.copy(name[0], "src/market_query/"+name[0].split('/')[4])
                '''
                if (name[0] in true_path):
                    aa = aa+1
                    print(aa)
                   
                    Grad_cams = Grad_Cam(model_I1,model_xencoder, [model_I1.module.layer4[-1].conv3], True, 'src/pa100k_origin_text_feature_early.npy')
                    grad_cam_list  = Grad_cams(imgs, name[0], sems[0])
                    cv2.imwrite("src/pa100k/origin_early/true/"+name[0].split('/')[4],grad_cam_list[0])
                '''
                
                '''
                if (name[0] not in false_path) and (name[0] in true_path):
                    aa = aa+1
                    print(aa)
    
                    Grad_cams = Grad_Cam(model_I1,model_xencoder, [model_I1.module.layer4[-1].conv3], True, 'src/pa100k_text_feature.npy')
                    grad_cam_list  = Grad_cams(imgs, name[0], sems[0])
                    cv2.imwrite("src/pa100k/gcn/false/"+name[0].split('/')[4],grad_cam_list[0])
                '''
        #global total
        #print(total)
        #np.save('src/extra_result.npy', result)   
        pdb.set_trace()
