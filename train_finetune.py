...
def main():
    ...
    if args.evaluate:
        ...
        for name in args.dataset:
            ...
            '''
            checkpoint = torch.load('/public/users/pengqy/log/peta-results/checkpoint_ep_jt_I1100.pth.tar')
            model_I1.module.load_state_dict(checkpoint['state_dict'])
           
            checkpoint = torch.load('/public/users/pengqy/log/peta-results/checkpoint_ep_jt_x100.pth.tar')
            model_xencoder.module.load_state_dict(checkpoint['state_dict']) 

            checkpoint = torch.load('/public/users/pengqy/log/peta-results/checkpoint_ep_jt_a100.pth.tar')
            model_aencoder.module.load_state_dict(checkpoint['state_dict'])   
                       
            checkpoint = torch.load('/public/users/pengqy/log/peta-results/checkpoint_ep_jt_G100.pth.tar')
            model_G.module.load_state_dict(checkpoint['state_dict']) 
            '''
    ...
    scheduler_G_alone = lr_scheduler.MultiStepLR(optimizer_G_alone, milestones=args.stepsize, gamma=args.gamma)
    scheduler_A_alone = lr_scheduler.MultiStepLR(optimizer_A_alone, milestones=args.stepsize_sal, gamma=args.gamma)
        ...
        if (epoch + 1) % args.save_pt  == 0 or  (rank1 > best_rank1):
            ...
            if not args.no_save:
                save_checkpoint({
                    ...
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_jt_I1' + str(epoch + 1) + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_I2,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_jt_I2' + str(epoch + 1) + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_I2_sem,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_jt_I2sem' + str(epoch + 1) + '.pth.tar'))
                ...
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_jt_G' + str(epoch + 1) + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_xencoder,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_jt_x' + str(epoch + 1) + '.pth.tar'))

                save_checkpoint({
                    'state_dict': state_dict_aencoder,
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(args.save_dir, 'checkpoint_ep_jt_a' + str(epoch + 1) + '.pth.tar'))
    ...
    for epoch in range(args.start_epoch_al, args.max_epoch_al):
        ...
        if (epoch + 1) % args.save_pt == 0 or (rank1 > best_rank1):
            ...
            save_checkpoint({
                ...
                'rank1': rank1,
                'epoch': epoch,
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_I1' + str(epoch + 1) + '.pth.tar'))

            save_checkpoint({
                'state_dict': state_dict_G,
                'rank1': rank1,
                'epoch': epoch,
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_G' + str(epoch + 1) + '.pth.tar'))

            save_checkpoint({
                'state_dict': state_dict_I2,
                'rank1': rank1,
                'epoch': epoch,
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_I2' + str(epoch + 1) + '.pth.tar'))

            save_checkpoint({
                'state_dict': state_dict_I2_sem,
                'rank1': rank1,
                'epoch': epoch,
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_I2sem' + str(epoch + 1) + '.pth.tar'))
            ...
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_x' + str(epoch + 1) + '.pth.tar'))
            ...
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_a' + str(epoch + 1) + '.pth.tar'))

            save_checkpoint({
                'state_dict': state_dict_GX,
                'rank1': rank1,
                'epoch': epoch,
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_GX' + str(epoch + 1) + '.pth.tar'))

            save_checkpoint({
                'state_dict': state_dict_GA,
                'rank1': rank1,
                'epoch': epoch,
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_GA' + str(epoch + 1) + '.pth.tar'))
            ...
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_DS' + str(epoch + 1) + '.pth.tar'))
            ...
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep_al_DC' + str(epoch + 1) + '.pth.tar'))
