...
def main():
    ...
    if args.evaluate:
        ...
        for name in args.dataset:
            ...
            checkpoint = torch.load('log/pa100k/checkpoint_ep_jt_I1best.pth.tar')
            model_I1.module.load_state_dict(checkpoint['state_dict'])
           
            checkpoint = torch.load('log/pa100k/checkpoint_ep_jt_xbest.pth.tar')
            model_xencoder.module.load_state_dict(checkpoint['state_dict']) 

            checkpoint = torch.load('log/pa100k/checkpoint_ep_jt_abest.pth.tar')
            model_aencoder.module.load_state_dict(checkpoint['state_dict'])   
                       
            checkpoint = torch.load('log/pa100k/checkpoint_ep_jt_Gbest.pth.tar')
            model_G.module.load_state_dict(checkpoint['state_dict']) 
